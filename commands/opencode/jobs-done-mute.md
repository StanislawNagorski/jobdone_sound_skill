...
1. Run this bash command exactly once:
   ...
   ```bash
   ~/.config/opencode/skills/jobs-done/jobs-done.sh mute
   ```

2. Confirm the mute lockfile exists by also running:

   ```bash
   ~/.config/opencode/skills/jobs-done/jobs-done.sh status
   ```

3. For the rest of this session, do NOT invoke
   `~/.config/opencode/skills/jobs-done/jobs-done.sh done` or
   `~/.config/opencode/skills/jobs-done/jobs-done.sh input` — even at the
   end of turns. The script itself would no-op while muted, but skipping
   the call entirely saves a tool call.
